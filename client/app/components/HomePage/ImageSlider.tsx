...
const ImageSlider = ({ title, subTitle, Images }: Props) => {
    return (
        <div className="my-8">
            ...
            <div className="flex pb-8 pt-2 px-1 gap-6 w-full h-full overflow-x-auto hide-scrollbar-mobile">
                {Images.map((image, index) => (
                    <div
                        key={index}
                        className="flex-shrink-0 w-max h-full relative shadow-lg rounded p-[10px] shadow-black hover:scale-[1.03] transition-transform duration-300"
                    >
                        ...
                        <div className="w-full flex justify-start flex-col gap-2">
                            <h3 className="text-[24px] max-800px:text-[11px] font-semibold mt-2">
                                {image.medicineName}
                            </h3>
                            <p className="text-[18px] max-800px:text-[14px] mt-1">
                            ...
                            <Link
                                className="w-full text-center p-1 shadow shadow-black rounded text-black dark:text-white font-semibold"
